...
export async function post(endpoint: string, params: Record<string, any> = {}, init?: RequestInitWithParams): Promise<any> {
  ...
  const res = await fetchWithParams(SERVER_CONFIG.AQUA_HOST + endpoint, {
    method: 'POST',
    params,
    ...init
  }).catch(e => {
...
async function logout(user: { token: string }) {
  const data = await post('/api/v2/user/logout', user)
  localStorage.removeItem("token");
  window.location.href = "/";
}
...
export const DATA = {
  allMusic: (game: GameName): Promise<AllMusic> =>
    ...
    fetch(`https://net.xn--igrr70arr3c.vip/assets/data/data.json`).then(it => it.json()),
