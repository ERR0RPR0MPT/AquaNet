...
import { SERVER_CONFIG, DEFAULT_PFP } from "./config"
import type { AquaNetUser } from "./generalTypes"

export function title(t: string) {
  document.title = `AquaNet via ${SERVER_CONFIG.SERVER_NAME} - ${t}`
}
...
export function pfp(node: HTMLImageElement, me?: AquaNetUser) {
  node.src = me?.profilePicture ? `${SERVER_CONFIG.AQUA_HOST}/uploads/net/portrait/${me.profilePicture}` : DEFAULT_PFP
