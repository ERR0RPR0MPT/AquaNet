...
<script lang="ts">
  ...
  import {DISCORD_INVITE, FADE_IN, FADE_OUT, SERVER_CONFIG} from "../../libs/config";
  ...
  function getStarted() {
    ...
    USER.keychip().then(k => {
      ...
      codeToHtml(`
[dns]
default=${SERVER_CONFIG.AQUA_CONNECTION}
    ...
    <div transition:slide>
      ...
      {:else}
        <div class="no-margin" in:fade={FADE_IN}>
          ...
          <p>
            {t('home.setup.ask')} <a href={SERVER_CONFIG.DISCORD_INVITE}>Discord</a> {t('home.setup.support')}.
          </p>
        </div>
      {/if}
    </div>
