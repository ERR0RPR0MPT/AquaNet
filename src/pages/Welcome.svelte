<script lang="ts">
  ...
  import {SERVER_CONFIG, saveServerConfig, TURNSTILE_SITE_KEY} from "../libs/config";

  let params = new URLSearchParams(window.location.search)

  let state = "home"
  $: isSignup = state === "signup"
  $: SERVER_CONFIG, updateServerConfig(SERVER_CONFIG);
  ...
  let updateServerConfig = (config: typeof SERVER_CONFIG) => {
    SERVER_CONFIG.SERVER_NAME = config.SERVER_NAME
    SERVER_CONFIG.AQUA_HOST = config.AQUA_HOST
    SERVER_CONFIG.DATA_HOST = config.DATA_HOST
    SERVER_CONFIG.AQUA_CONNECTION = config.AQUA_CONNECTION
    saveServerConfig({
      ...SERVER_CONFIG,
      SERVER_NAME: config.SERVER_NAME,
      AQUA_HOST: config.AQUA_HOST,
      DATA_HOST: config.DATA_HOST,
      AQUA_CONNECTION: config.AQUA_CONNECTION,
    });
  }
...
<main id="home" class="no-margin">
  <div>
    <!--    <h1 id="title">AquaNet <br>via <br>{SERVER_CONFIG.SERVER_NAME}</h1>-->

    <h1 id="title">AquaNet</h1>
    <h1 id="title">via</h1>
    <h1 id="title">{SERVER_CONFIG.SERVER_NAME}</h1>
    ...
    {:else if state === "login" || state === "signup"}
      <div class="login-form" transition:slide>
        ...
        <input type="aqua-host" placeholder={t('aqua-host')} bind:value={SERVER_CONFIG.AQUA_HOST}>
        <input type="server-name" placeholder={t('server-name')} bind:value={SERVER_CONFIG.SERVER_NAME}>
        ...
        <button on:click={() => {
          ...
          {t('welcome.btn-server-mai2-link')}
        </button>

        <button on:click={() => {
          updateServerConfig({
            SERVER_NAME: "AquaDX",
            AQUA_HOST: "https://aquadx.net/aqua",
            DATA_HOST: "https://aquadx.net",
            AQUA_CONNECTION: "aquadx.hydev.org"
          });
        }
        }>
          {t('welcome.btn-server-aquadx')}
        </button>

        <button on:click={() => {
          updateServerConfig({
            SERVER_NAME: "Local",
            AQUA_HOST: "http://localhost/aquanet",
            DATA_HOST: "https://aquadx.net",
            AQUA_CONNECTION: "{your_own_server_ip}"
          });
        }
        }>
