...
<script lang="ts">
  ...
  import {FADE_IN, FADE_OUT, SERVER_CONFIG, USERBOX_DEFAULT_URL} from "../../libs/config";
  ...
  function exportData() {
    submitting = "export"
    GAME.export('chu3')
      .then(data => download(JSON.stringify(data), SERVER_CONFIG.SERVER_NAME+`_chu3_export_${userbox.userName}.json`))
      .catch(e => error = e.message)
      .finally(() => submitting = "")
  }
